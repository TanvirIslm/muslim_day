...
class _HijriCalendarScreenState extends State<HijriCalendarScreen> {
  ...
  void _changeMonth(int delta) {
    setState(() {
      DateTime currentGDate = _currentHijri.hijriToGregorian(
          _currentHijri.hYear, _currentHijri.hMonth, 1);
      DateTime newGDate =
          DateTime(currentGDate.year, currentGDate.month + delta, 1);
  ...
  Widget build(BuildContext context) {
    final int daysInMonth =
        _getDaysInHijriMonth(_currentHijri.hYear, _currentHijri.hMonth);

    // Safely calculate the first day of the month
    final HijriCalendar firstDayOfMonth = HijriCalendar()
      ..hYear = _currentHijri.hYear
      ..hMonth = _currentHijri.hMonth
      ..hDay = 1;
    final DateTime gregorianFirst = firstDayOfMonth.hijriToGregorian(
        firstDayOfMonth.hYear, firstDayOfMonth.hMonth, 1);
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                if (index < weekdayOffset)
                  return const SizedBox(); // Empty space for correct alignment
    ...
    return InkWell(
      onTap: () {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              "${_toBengaliNumber(day)} ${_currentHijri.longMonthName} | ${gregorianDate.day}-${gregorianDate.month}-${gregorianDate.year}"),
          backgroundColor: Theme.of(context).primaryColor,
        ));
      },
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(_toBengaliNumber(day),
                style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: isToday ? Colors.white : Colors.black)),
            Text("${gregorianDate.day}",
                style: TextStyle(
                    fontSize: 10,
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              children: [
                Text(_currentHijri.longMonthName,
                    style: GoogleFonts.amiri(
                        fontSize: 24,
                        color: Colors.white,
                        fontWeight: FontWeight.bold)),
                Text("${_toBengaliNumber(_currentHijri.hYear)} হিজরি",
