...
class _AmalJournalPageState extends State<AmalJournalPage> {
  ...
  Future<void> _generateAndSavePDF(AmalProvider provider, int completed, double progress, Color appPrimaryColor) async {
    try {
      ...
      List<pw.Widget> pdfHeatmapSquares = _last30Days.map((date) {
        ...
        if (dailyProg > 0 && dailyProg <= 0.33) squareColor = PdfColor.fromHex('#A3D5C7');
        if (dailyProg > 0.33 && dailyProg <= 0.66) squareColor = PdfColor.fromHex('#4DB499');
        if (dailyProg > 0.66) squareColor = primaryPdfColor;
        
        return pw.Container(width: 12, height: 12, decoration: pw.BoxDecoration(color: squareColor, borderRadius: pw.BorderRadius.circular(2)));
      }).toList();
            ...
            return pw.Column(
              ...
              children: [
                pw.Container(
                  ...
                  decoration: pw.BoxDecoration(color: primaryPdfColor, borderRadius: pw.BorderRadius.circular(12)),
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.center,
                    children: [
                      pw.Text("Monthly Amal Overview", style: pw.TextStyle(color: PdfColors.white, fontSize: 26, fontWeight: pw.FontWeight.bold)),
                      pw.SizedBox(height: 8),
                      pw.Text("Your personal progress and consistency tracking.", style: pw.TextStyle(color: PdfColors.white, fontSize: 14)),
                    ],
                  ),
                ),
                ...
                pw.Text("30-Day Contribution Graph", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: PdfColors.black)),
                ...
                pw.Container(
                  ...
                  child: pw.Wrap(spacing: 4, runSpacing: 4, children: pdfHeatmapSquares),
                ),
                pw.SizedBox(height: 25),
                ...
                pw.Row(
                  ...
                  children: [
                    _buildPdfStatCard("Total Points", "${provider.totalPoints}", primaryPdfColor),
                    _buildPdfStatCard("Highest Streak", "${provider.getCategoryStreak('fardh')} Days", accentOrange), 
                  ],
                ),
                pw.SizedBox(height: 30),
                
                pw.Text("Category Summary", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: PdfColors.black)),
                pw.SizedBox(height: 10),
                pw.Table.fromTextArray(
                  ...
                  data: [
                    ['Fardh Worship', '${provider.getCategoryStreak('fardh')} Days', 'Active'],
                    ['Sunnah & Dhikr', '${provider.getCategoryStreak('sunnah')} Days', 'Active'],
                  ],
                ),
                pw.Spacer(),
                pw.Divider(color: PdfColors.grey400),
                pw.SizedBox(height: 10),
                pw.Text("Generated on: ${DateFormat('MMMM d, yyyy').format(DateTime.now())}", style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey700)),
              ],
            );
          },
        ),
  ...
  pw.Widget _buildPdfStatCard(String title, String value, PdfColor color) {
    return pw.Container(
      ...
      decoration: pw.BoxDecoration(color: PdfColors.white, borderRadius: pw.BorderRadius.circular(8), border: pw.Border.all(color: color, width: 1.5)),
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.center,
        children: [
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("আমল ট্র্যাকার", style: GoogleFonts.notoSansBengali(fontWeight: FontWeight.bold, color: Colors.white)),
        ...
        actions: [
          ...
          Padding(
            ...
            child: Chip(
              ...
              label: Text(provider.totalPoints.toString(), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => _generateAndSavePDF(provider, completedAmals.length, currentProgress, primaryColor),
        backgroundColor: primaryColor,
        icon: const Icon(Icons.picture_as_pdf, color: Colors.white),
        label: Text("রিপোর্ট", style: GoogleFonts.notoSansBengali(fontWeight: FontWeight.bold, color: Colors.white)),
      ),
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        _selectedDate.day == DateTime.now().day ? "আজকের আমল" : DateFormat('d MMM, yyyy').format(_selectedDate),
                        style: GoogleFonts.notoSansBengali(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87),
                      ),
                      Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                ...
                                style: TextStyle(color: isSelected ? Colors.white70 : Colors.grey, fontSize: 11, fontWeight: FontWeight.w600),
                              ),
                              ...
                              if (dailyProg == 1.0)
                                Container(width: 4, height: 4, decoration: BoxDecoration(color: isSelected ? Colors.white : primaryColor, shape: BoxShape.circle)),
                            ],
                          ),
                        ),
                      );
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // --- 3. Amal Groups ---
                  _buildAmalGroup(context, provider, 'fardh', 'ফরজ ইবাদত', completedAmals, _selectedDate, primaryColor),
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text("গত ৩০ দিনের আমল", style: GoogleFonts.notoSansBengali(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
                Row(
                  children: [
                    Icon(Icons.local_fire_department, color: Colors.orange, size: 16),
                    const SizedBox(width: 4),
                    Text("${provider.getCategoryStreak('fardh')} দিন", style: GoogleFonts.notoSansBengali(fontWeight: FontWeight.bold, color: Colors.orange)),
                  ],
                )
              ],
            ),
            ...
            Row(
              ...
              children: [
                Text("কম", style: GoogleFonts.notoSansBengali(fontSize: 10, color: Colors.grey)),
                const SizedBox(width: 4),
                Container(width: 10, height: 10, decoration: BoxDecoration(color: Colors.grey.shade200, borderRadius: BorderRadius.circular(2))),
                const SizedBox(width: 4),
                Container(width: 10, height: 10, decoration: BoxDecoration(color: primaryColor.withValues(alpha: 0.3), borderRadius: BorderRadius.circular(2))),
                const SizedBox(width: 4),
                Container(width: 10, height: 10, decoration: BoxDecoration(color: primaryColor.withValues(alpha: 0.6), borderRadius: BorderRadius.circular(2))),
                const SizedBox(width: 4),
                Container(width: 10, height: 10, decoration: BoxDecoration(color: primaryColor, borderRadius: BorderRadius.circular(2))),
                const SizedBox(width: 4),
                Text("বেশি", style: GoogleFonts.notoSansBengali(fontSize: 10, color: Colors.grey)),
              ],
            )
          ],
        ),
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text(title, style: GoogleFonts.notoSansBengali(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black54)),
          ),
          ...amals.map((amal) {
    ...
    return Padding(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: isDone ? primaryColor.withValues(alpha: 0.3) : Colors.grey.shade200),
        ),
        child: InkWell(
          onTap: onTap,
          borderRadius: BorderRadius.circular(16),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: isDone ? primaryColor.withValues(alpha: 0.15) : Colors.grey.shade100, shape: BoxShape.circle),
                  child: Icon(amal.icon, size: 22, color: isDone ? primaryColor : Colors.grey.shade500),
                ),
